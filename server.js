...
app.post('/adicionarProduto', async (req, res) => {
    try {
        ...
        console.log("cartao passado:", {clienteCartao}, "itens passados:", {id_servico})
...
app.post('/confirmarServicos', async (req, res) => {
    try {
        ...
        await conexao.execute(
            `UPDATE produtos SET flag = 0 WHERE id IN (${placeholders})`,
            produtos
        );

        // Conte o número de produtos alterados
        const numProdutosAlterados = produtos.length;

        // Consulte o cliente_cartao a partir dos produtos
        console.log("IDs de produtos na consulta:", produtos);

        const [clienteCartaoConsulta] = await conexao.execute(
            `SELECT DISTINCT cliente_cartao FROM produtos WHERE id IN(${placeholders})`,
            produtos
        );

        console.log("Após a consulta cliente_cartao:", clienteCartaoConsulta);

        if (!clienteCartaoConsulta || clienteCartaoConsulta.length === 0) {
            console.error("Erro: cliente_cartao não encontrado ou duplicado.");
            return res.status(500).json({
                success: false,
                message: 'Erro ao obter o cliente_cartao.'
            });
        }

        const clienteCartao = clienteCartaoConsulta[0].cliente_cartao;

        // Chame a função para confirmar recompensas, passando o clienteCartao como parâmetro
        await confirmarRecompensas(clienteCartao, numProdutosAlterados);

        // Recupere os produtos após a atualização
        const [resultadosAtualizados] = await conexao.execute(
            'SELECT id, flag FROM produtos WHERE id IN (?)',
            [produtos]
        );

        console.log("Todos os produtos após a atualização:");
        resultadosAtualizados.forEach((produto) => {
            console.log(`ID: ${produto.id}, Flag: ${produto.flag}`);
        });
